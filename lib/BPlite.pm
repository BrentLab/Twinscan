...
sub nextHSP {
	...
	return undef if $nextline =~ /^\s*$/; #wei 3/14/2004 for multi-report alignment results
	$scoreline .= $nextline;
	my ($score, $bits);
	if ($scoreline =~ /\d bits\)/) {
	...
	while(<$FH>) {
	    #die if /^BLAST/;
		last if /^BLAST/; # wei, 3/14/2004 for multi-report alignment result
	...
	my $hsp = BPlite::HSP::new( $score, $bits, $match, $positive, $length, $p,
		$qb, $qe, $sb, $se, $ql, $sl, $as, $qgaps, $sgaps, $sbjct);
	return $hsp;
}
...
sub new {
	...
	($hsp->{SCORE}, $hsp->{BITS},
		...
		$hsp->{QL}, $hsp->{SL}, $hsp->{AS}, $hsp->{QG}, $hsp->{SG}, $hsp->{SBJT}) = @_;
